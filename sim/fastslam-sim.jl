...
function pfsim_setup(n_landmarks::Integer, waypoints_file::AbstractString)
    ...
    # SLAM state vector
    state = PFSlamState(vehicle.pose, zeros(3, 3))
...
function pfsim!(simdata::SimData)
    sim!(simdata::SimData, (x...) -> (), [])
end


function pfsim!(simdata::SimData, monitor::Function, args::AbstractVector)
    ...
    while vehicle.waypoint_id != 0
        ...
        # Prediction update for particles
        state.particles = predict(state, vehicle, dt)

        dtsum += dt

        # PF update step - takes place every (dt_obs/dt) timesteps
        if dtsum > dt_obs
            dtsum = 0
            ...
            zf, idf, zn, state.da_table = associate_known(state, simdata.z, tags)
            ...
            # Observe new features and augment map
            if size(zn, 2) > 0
                # Sample again if necessary
                # Augment map with any new features
            end
